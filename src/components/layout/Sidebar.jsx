    ...
    <aside className="w-60 min-h-screen bg-white border-r border-slate-100 flex flex-col shrink-0">
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {NAV.filter(n => !n.perm || can(n.perm)).map(n => (
          <NavLink key={n.to} to={n.to} className={({ isActive }) => 'sidebar-item ' + (isActive ? 'sidebar-active' : '')}>
      ...
      <div className="px-3 py-3 border-t border-slate-100">
        <div className="flex items-center gap-3 px-3 py-2 rounded-xl bg-slate-50">
          ...
          <button onClick={handleLogout} title="Logout" className="btn-icon text-red-400 hover:text-red-600 hover:bg-red-50">⏏</button>
        </div>
      </div>
    </aside>
  );
